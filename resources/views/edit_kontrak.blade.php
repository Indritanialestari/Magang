    ...
    <div class="container mx-auto bg-white p-6 rounded-lg shadow-md max-w-2xl">
        ...
        <form action="{{ route('karyawan-kontrak.update', $karyawanKontrak->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {{-- Fields khusus Karyawan Kontrak (termasuk yang diubah) --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 col-span-full">
                    
                    {{-- [PERUBAHAN] Input Masa Kerja diubah menjadi tipe number agar bisa desimal --}}
                    <div>
                        <label for="masa_kerja" class="block text-sm font-medium text-gray-700">Masa Kerja (Tahun)</label>
                        <input type="number" step="0.1" name="masa_kerja" id="masa_kerja" value="{{ old('masa_kerja', $karyawanKontrak->masa_kerja) }}"
            ...
            <div class="mt-6 flex justify-end space-x-2">
                <a href="{{ route('karyawan-kontrak.exportDetail', $karyawanKontrak->id) }}" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                    Download PDF
                </a>
